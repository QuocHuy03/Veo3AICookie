...
class VideoProcessingThread(QThread):
    """Thread để xử lý video không block UI với parallel processing"""
    progress_updated = pyqtSignal(int, str)  # progress, message
    status_updated = pyqtSignal(str)  # status message for log
    finished = pyqtSignal(list)  # results
    
    def __init__(self, prompts, account_data, config, max_workers=3):
        super().__init__()
    ...
    def process_single_video(self, prompt_data):
        ...
        try:
            # Lấy token từ cookie
            cookie_header_value = self.account_data["cookie"]
            token = fetch_access_token_from_session(cookie_header_value)
            
            if not token:
                return (stt, prompt, False, "Không thể lấy token")
    ...
    def run(self):
        try:
            self.progress_updated.emit(5, f"🚀 Bắt đầu xử lý {self.total_count} video với {self.max_workers} luồng...")
...
class MainWindow(QMainWindow):
    ...
    def create_account_tab(self):
        ...
        self.account_table.setHorizontalHeaderLabels(["NAME", "EMAIL", "STATUS", "EXPIRES"])
    ...
    def create_processing_tab(self):
        ...
        self.account_info_label = QLabel("Chưa chọn tài khoản")
        self.account_info_label.setStyleSheet("color: gray; font-size: 12px;")
    ...
    def load_accounts_to_table(self):
        ...
        header.setSectionResizeMode(3, header.ResizeToContents)  # CREATED_AT
    ...
    def update_account_combo(self):
        ...
        for account in self.accounts:
            self.account_combo.addItem(account.get("name", "Unknown"))
            
    def on_account_changed(self, account_name):
        """Khi chọn account khác"""
        if not account_name:
            self.account_info_label.setText("Chưa chọn tài khoản")
            return
        ...
        for account in self.accounts:
            if account.get("name") == account_name:
                ...
                self.account_info_label.setText(f"Trạng thái: {status}\nTạo: {created}")
                break
    ...
    def start_processing(self):
        ...
        if not self.account_combo.currentText():
            create_styled_messagebox(self, "Lỗi", "Vui lòng chọn tài khoản!", QMessageBox.Warning).exec_()
            return
        ...
        if not account_data:
            create_styled_messagebox(self, "Lỗi", "Không tìm thấy dữ liệu tài khoản!", QMessageBox.Warning).exec_()
            return
        ...
        # Start processing thread với parallel processing
        max_workers = self.max_workers_spin.value()
        self.processing_thread = VideoProcessingThread(prompts, account_data, config, max_workers)
...
def main():
    ...
    app.setApplicationName("VEO3 AI Video Generator")
